...
function BankRow({ name, email, address, contact, userId, bankId, formData }) {
  ...
  const handleRequestBlood = async () => {
    ...
    if (!request) {
      const confirmed = window.confirm(
        "Are you sure you want to proceed with the blood request?"
      );
      if (confirmed) {
        try {
  ...
  return (
    <tr>
      ...
      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
        <span className="ml-3 font-bold text-white"> {address} </span>
      </td>
      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
        <span className="ml-3 font-bold text-white">
          ...
          <button
            ...
            class="px-3 py-2 text-xs font-medium text-center text-white bg-red-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          >
            Request blood
          </button>{" "}
        </span>
